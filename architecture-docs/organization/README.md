...
- AWS Organizations is an optimal way to manage and govern several accounts for an environment at this scale in one place and it is free to use. AWS Organizations makes it easy for administrators to create permission boundaries for accounts and OUs using SCPs, streamlining security operations to keep the organization in line with best security practices. AWS Organizations also includes consolidated billing for all accounts so you can receive one bill which can result in cost savings.
...
- The organization is separated into three OUs (plus the management account) to separate security operations, infrastructure management, and workloads. Each of these OUs holds accounts with similar duties which will make adding guardrails more efficient.
- Adding SCPs to OUs instead of individual accounts reduces the number of policies administrators have to manage and provides easier onboarding and offboarding for accounts.
- The Infrastructure OU holds five different accounts that are used for different infrastructure purposes. The isolation makes it easier for teams to be given access only to accounts that are aligned with their specific duties. Since each account manages infrastructure, they were all added to the same OU.
