...
All application containers running in ECS tasks send their logs to Amazon CloudWatch Logs for monitoring and troubleshooting. Metrics are taken from the containers, supporting network resources, and databases for CloudWatch Alarms to watch. When an alarm is triggered, CloudWatch Alarms will send notifications directly to Amazon SNS for teams to be alerted. CloudWatch Dashboards will provide a centralized view of key metrics across workloads to help teams collaborate and respond to incidents effectively.
